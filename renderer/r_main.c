...
{	
	...
	Com_Printf("R_Init: got gl_driver: %s\n",gl_driver->string);
	...
	VID_Printf (PRINT_ALL, "Init:GL_MAX_TEXTURE_SIZE: %i\n", gl_config.max_texsize );

	gl_config.extensions_string = qglGetString (GL_EXTENSIONS);
	Com_Printf (PRINT_ALL, "GL_EXTENSIONS: %s\n", gl_config.extensions_string );
	//Gfx cards so big that they break VID_Printf now!
	...
	// 3DFX is long dead. Cope with it!
...
void R_BeginFrame( float camera_separation )
{
	...
	// Make the breakage more transparent
	
	if (!con_font)
	  Com_Printf("Font cvar not initialised\n");

	// Knightmare- added Psychospaz's console font size option
	if (con_font->modified)
	{
		Com_Printf("Font: %s %d\n", con_font->string, con_font_size->value);
		RefreshFont ();
	}
