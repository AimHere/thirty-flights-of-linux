...
static qboolean mouse_reset = false;
...
void RW_IN_Init(in_state_t *in_state_p)
{
	...
	Com_Printf("RW_IN_Init: in_state = %p\n");
}
		...
		case MotionNotify:
			if (mouse_active) {
				...
				if (dgamouse) {
					if (mouse_reset)
					{
						my += event.xmotion.x*2;
						my += event.xmotion.y*2;
						mouse_reset = false;
...
int GLimp_SetMode( int *pwidth, int *pheight, int mode, qboolean fullscreen )
{
	...
	if (!XF86VidModeQueryVersion(dpy, &MajorVersion, &MinorVersion)) { 
		...
		Com_Printf(PRINT_ALL, "Using XFree86-VidModeExtension Version %d.%d\n",
