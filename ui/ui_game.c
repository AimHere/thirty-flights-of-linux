...
int which_game = GAME_THIRTYFLIGHTS;
...
void Game_MenuInit( void )
{
	...
	if (!Alias_Exists("gravitybonegame")) 
	{
		Com_Printf("Detected: Gravity Bone\n");
		GravityBone_MenuInit();
	} else
	{
		Com_Printf("Detected: Thirty Flights\n");
		ThirtyFlights_MenuInit();
	}
	
}
	

void ThirtyFlights_MenuInit( void )
{
...
void GravityBone_MenuInit( void )
{
	static const char *yesno_names[] =
	{
		"no", "yes", 0
	...
	s_medium_game_action.generic.x		= 0;
	s_medium_game_action.generic.y		= y += MENU_LINE_SIZE;
	s_medium_game_action.generic.name	= "Play Gravity Bone";
	s_medium_game_action.generic.iconname	= "start";
	s_medium_game_action.generic.callback = MediumGameFunc;
