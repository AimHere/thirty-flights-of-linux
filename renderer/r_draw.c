...
void RefreshFont (void)
{
	...
	Com_Printf("Attempted to find font %s.png\n",con_font->string);

	if (!draw_chars) // fall back on default font
	{
		Com_Printf("Falling to default.pcx\n");
		draw_chars = R_FindImage ("fonts/default.pcx", it_pic);
	}
	if (!draw_chars) // fall back default.tga (Standalone Gravity Bone needs this)
	{
		Com_Printf("Falling to default.tga\n");
		draw_chars = R_FindImage ("fonts/default.tga", it_pic);
	}

	if (!draw_chars) // fall back on old Q2 conchars
	{
		Com_Printf("Falling to Q2 Conchars\n");
